...
class PetHealthCalculator:
    ...
    def calculate_pet_health(
        ...
        try:
            ...
            water_score = self._calculate_water_score(
                water_summary.get("total_ml", 0), user_goals.get("daily_water_ml", 2000)
            )
    ...
    def _calculate_food_score(
        self, actual_calories: float, goal_calories: float
    ) -> float:
        """
    ...
    def _calculate_exercise_score(
        self, workout_count: int, cardio_minutes: float
    ) -> float:
        """
    ...
    def _generate_recommendations(
        self, water_score: float, food_score: float, exercise_score: float
    ) -> list:
        """
        ...
        if food_score < 70:
            if food_score < 40:
                recommendations.append(
                    "🍽️ Try to eat more balanced meals and reach your calorie goal."
                )
            else:
                recommendations.append(
                    "🍎 Pay attention to your nutrition and meal balance."
                )

        if exercise_score < 70:
            if exercise_score == 0:
                recommendations.append(
                    "💪 Start with any exercise - even a 15-minute walk helps!"
                )
            elif exercise_score < 50:
                recommendations.append(
                    "🏃 Add more workouts to your routine - aim for at least 30 minutes."
                )
            else:
                recommendations.append(
                    "🎯 You're close! Try to fit in one more workout today."
                )

        if not recommendations:
            recommendations.append(
                "✨ You're doing great! Keep maintaining these healthy habits!"
            )
    ...
    def calculate_weekly_trend(self, daily_health_scores: list) -> Dict[str, Any]:
        """
        ...
        if len(daily_health_scores) > 1:
            variance = sum((x - average) ** 2 for x in daily_health_scores) / len(
                daily_health_scores
            )
